...
typedef struct _tWasm_value_stack {
    tWasm_value *base;
    uint16_t size;
    uint16_t top;
} tWasm_value_stack;
...
struct _tWasm_function;

typedef enum _eWasm_stack_item_type {
    WASM_LOCAL, WASM_FRAME
} eWasm_stack_item_type;

typedef struct _tWasm_frame {
    struct _tWasm_frame *caller_frame;
    uint16_t locals_count;
    tWasm_value_stack value_stack;
    const struct _tWasm_function *function;
    tWasm_code_pointer instruction_counter;
} tWasm_frame;

typedef struct _tWasm_stack_item {
...
typedef struct _tWasm_signature {
    eWasm_value_type return_type;
    unsigned param_count;
    eWasm_value_type* params;
} tWasm_signature;

typedef struct _tWasm_entity_name {
    const char *name;
    uint8_t name_length;
} tWasm_entity_name;

typedef enum _eWasm_binding_type {
    WASM_BINDING_TYPE_NONE, WASM_BINDING_TYPE_U32, WASM_BINDING_TYPE_BYTE_ARRAY
} eWasm_binding_type;

typedef struct _tWasm_binding_value {
...
typedef struct _tWasm_function_call {
    const char *name;
    unsigned param_count;
...
typedef struct _tWasm_binding {
    tWasm_function_call call_ctx;
    rWasm_function_binding routine;
} tWasm_binding;

typedef struct _tWasm_function {
...
typedef struct _tWasm_function_list_item {
    tWasm_function function;
    struct _tWasm_function_list_item *next;
} tWasm_function_list_item;

typedef struct _tWasm_function_list {
